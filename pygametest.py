...
def main():
    ...
    allMoves = [] #list of all move made from the solved configuration.
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        mainBoard, solutionSeq = generateNewPuzzle(80) #click on new game button
                        ...
                        # check if the clicked tiles was next to the blank spot
...
def checkForQuit():
    for event in pygame.event.get(QUIT):#get all the quit event
...
def getStartingBoard():
    ...
    for x in range (BOARDWIDTH):
        ...
        board.append(column)#inserting the number inside the blue line

        counter -= BOARDWIDTH * (BOARDHEIGTH - 1) + BOARDWIDTH -1

    board [BOARDWIDTH - 1] [BOARDHEIGTH - 1] = None
    return board
...
def isValidMove(board, move):
    # != (not equal) len = (The len() function returns the number of items in an object.)
...
def drawBoard(board, message):
    ...
    if message:
        textSurf, textRect = makeText(message, MASSAGECOLOR, BGCOLOR, 5, 5)
...
def slideAnimation(board, direction, message, animationSpeed): #connecting to getRamndom function
...
def resetAnimation(board, allMoves):
    ...
    revAllMoves = allMoves[:] #get the copy of the list
    revAllMoves.reverse()
